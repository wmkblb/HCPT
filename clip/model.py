...
class ResidualAttentionBlock_IVLP(nn.Module):  # 独立的VL模型
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
        ...
        '''
        只有当标志设置为 True 时才添加可学习的标记（learnable tokens）。add_prompt
        对于第一次迭代（i=0），我们不应添加可学习的参数，因为在一开始的时候已经处理了文本和视觉分支的可学习标记。
        '''
        self.text_layer = text_layer  # 这是一个bool值 ， 代表是不是文本层
        self.attn_mask = attn_mask
        if i != 0:
            self.add_prompt = add_prompt
            if self.add_prompt:
                if self.text_layer:
                    self.n_ctx_text = design_details["language_ctx"]  # hyperparameter 提示文本长度
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.add_prompt:
            ...
            # 还要看看这是文本变换器层还是其他类型的层。
            if not self.text_layer:
                # Remove the outputs produced by learnable tokens of previous layer
                # 移除前一层可学习标记产生的输出。
                prefix = x[0:x.shape[0] - self.n_ctx_visual, :, :]  # 这是去掉了提示
                ...
                # 通过将本层的可学习标记与输入相加，替换前一层的可学习标记。
                x = torch.cat([prefix, visual_context], dim=0)  # 前缀加新的可学习的提示词 前缀包括cls加图片patch
            else:
                ...
                # layer learnable tokens
                x = torch.cat([prefix, textual_context, suffix], dim=0)  # sos n_txt cls eos

        x = x + self.attention(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x))
        return x

class ResidualAttentionBlock_HCPT(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        self.compound_prompt_nctx = design_details['hcpt_length']  # 混合提示长度
        if self.i == 0:
            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:  # 视觉
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        # layer learnable tokens
                        x = torch.cat([prefix, visual_context], dim=0) # l bs dim
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'HCPT':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_HCPT(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        '''
        超参数决定是否需要在转换器块的输入中添加提示嵌入（prompt embeddings）
        '''
        self.prompt_till_layer_visual = design_details["vision_depth"]  # 提示的深度
...
class VisionTransformer_HCPT(nn.Module):
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "HCPT":
                self.visual = VisionTransformer_HCPT(
                    input_resolution=image_resolution,
    ...
    def encode_text(self, text):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND   N：bs  L：length D：dim
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
